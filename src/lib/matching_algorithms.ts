...
export function matchSchemes(farmerProfile: {
  ...
  const matchedSchemes = schemesData.schemes
    .map((scheme) => {
      ...
      if (
        ...
        const proximityBonus = Math.max(0, 5 * (1 - distanceFromMid / (landRange / 2)));
        
        score += 25 + Math.floor(proximityBonus);
        reasons.push(`Your land size (${farmerProfile.landholding}ha) matches perfectly`);
        ...
        if (overflow <= 1) {
          score += 10;
          reasons.push(`Slightly above land limit (${scheme.maxLandholding}ha)`);
        } else {
      ...
      if (
        ...
        if (incomeRatio <= 0.5) {
          ...
          reasons.push(`Income (₹${(farmerProfile.annualIncome / 1000).toFixed(0)}K) qualifies`);
        }
        ...
        if (overflow <= 50000) {
          ...
          reasons.push(`Income exceeds limit (max ₹${(scheme.maxIncome / 1000).toFixed(0)}K)`);
        }
      }
      ...
      if (
        ...
        if (farmerProfile.age < 40 && scheme.eligibleCategories.includes('young_farmer')) {
          score += 10;
          reasons.push('Young farmer benefit');
        } else if (farmerProfile.age > 60 && scheme.eligibleCategories.includes('senior')) {
          score += 10;
  ...
  return {
    ...
    accuracy: matchedSchemes.length > 0 ? Math.min(92, 85 + matchedSchemes.length) : 0,
  };
}
...
export function predictInsuranceNeeds(farmerProfile: {
  ...
  const riskFactors: {
    [key: string]: { weatherRisk: 'high' | 'medium' | 'low'; droughtProne: boolean; floodProne: boolean };
  } = {
    ...
    uttar_pradesh: { weatherRisk: 'medium', droughtProne: false, floodProne: true },
    gujarat: { weatherRisk: 'high', droughtProne: true, floodProne: false },
    andhra_pradesh: { weatherRisk: 'high', droughtProne: true, floodProne: true },
    kerala: { weatherRisk: 'medium', droughtProne: false, floodProne: true },
  };

  const stateRisk = riskFactors[farmerProfile.state.toLowerCase().replace(' ', '_')] || {
    weatherRisk: 'medium',
    droughtProne: false,
    floodProne: false,
  };
  ...
  if (farmerProfile.crops.includes('rice') && stateRisk.floodProne) cropRisk += 8;
  if (farmerProfile.crops.includes('wheat') && stateRisk.droughtProne) cropRisk += 6;
  ...
  if (cropRisk > 25) {
    const relevantProducts = insuranceProductsData.insuranceProducts
      .filter((p) => {
        ...
        return farmerProfile.crops.some(c => p.crops?.includes(c.toLowerCase()));
      })
      .slice(0, 2);

    recommendations.push({
      type: 'crop_insurance',
      reason: `High risk crops (${farmerProfile.crops.join(', ')}) require protection`,
      products: relevantProducts.length > 0 ? relevantProducts : insuranceProductsData.insuranceProducts.filter(p => p.type === 'crop_insurance').slice(0, 2),
    });
  }
  ...
  if (farmerProfile.irrigationType === 'drip' || farmerProfile.irrigationType === 'sprinkler') {
  ...
  if (stateRisk.droughtProne || stateRisk.floodProne) {
    ...
    recommendations.push({
      ...
      products: insuranceProductsData.insuranceProducts.filter(
        (p) => p.type === 'weather_insurance' && 
        (stateRisk.droughtProne ? p.weatherRisk === 'drought' : p.weatherRisk === 'excessive_rainfall')
      ),
    });
  }
  ...
  if (farmerProfile.hasLivestock && farmerProfile.livestockCount! > 0) {
    ...
    recommendations.push({
      ...
      reason: `${farmerProfile.livestockCount} livestock assets worth ₹${(farmerProfile.livestockCount! * 50000).toLocaleString()}`,
      products: insuranceProductsData.insuranceProducts.filter(
  ...
  const riskMultiplier = riskPercentage > 70 ? 2.5 : riskPercentage > 50 ? 2.0 : riskPercentage > 30 ? 1.5 : 1.0;
...
export function recommendLoans(farmerProfile: {
  ...
  purpose: 'seeds' | 'equipment' | 'infrastructure' | 'emergency' | 'working_capital';
  ...
  const recommendations = loanProductsData.loanProducts
    .map((loan) => {
      ...
      if (
        ...
        if (amountRatio >= 0.2 && amountRatio <= 0.8) {
          ...
          positiveReasons.push(
            `Amount (₹${(farmerProfile.loanAmountNeeded / 1000).toFixed(0)}K) within range`
          );
        }
        ...
        if (deficit <= 25000) {
          score += 15;
          positiveReasons.push(`Close to minimum (₹${(deficit / 1000).toFixed(0)}K short)`);
        } else {
      ...
      const purposeMatch = loan.purpose.includes('any_agricultural') ||
                          loan.purpose.includes(farmerProfile.purpose) ||
                          (farmerProfile.purpose === 'seeds' && loan.purpose.includes('farm_inputs')) ||
                          (farmerProfile.purpose === 'working_capital' && loan.purpose.includes('any_agricultural'));
      
      if (purposeMatch) {
        score += 20;
        if (loan.purpose.includes(farmerProfile.purpose)) {
          positiveReasons.push(`Specifically designed for ${farmerProfile.purpose.replace('_', ' ')}`);
        } else {
          positiveReasons.push(`Suitable for ${farmerProfile.purpose.replace('_', ' ')}`);
        }
      } else {
        score += 5; // Partial credit for general agricultural loans
        disqualifyReasons.push(`Primary focus: ${loan.purpose[0].replace('_', ' ')}`);
      }
      ...
      if (
        ...
        if (farmerProfile.landholding <= 2 && (loan.id.includes('pm-kisan') || loan.id.includes('kcc'))) {
          score += 3;
          positiveReasons.push(`Priority for small farmers (${farmerProfile.landholding}ha)`);
        } else {
          positiveReasons.push(`Landholding (${farmerProfile.landholding}ha) qualifies`);
        }
      } else {
        const landRatio = farmerProfile.landholding / loan.eligibility.minLandholding;
        if (landRatio > 0.7) {
          score += 8;
      ...
      if (!loan.collateralRequired) {
        ...
        const requiredValue = farmerProfile.loanAmountNeeded * loan.collateralValue;
        if (farmerProfile.collateralValue! >= requiredValue) {
          score += 15;
          const collateralRatio = farmerProfile.collateralValue! / requiredValue;
          if (collateralRatio >= 1.5) {
            score += 3;
            positiveReasons.push('Strong collateral coverage - higher approval chance');
          } else {
          ...
          if (shortfall <= 50000) {
            ...
            disqualifyReasons.push(`Need ₹${(shortfall / 1000).toFixed(0)}K more collateral`);
          }
        }
      } else {
      ...
      if (loan.interestRate <= 5) {
        score += 10;
        positiveReasons.push(`Excellent rate: ${loan.interestRate}% (subsidized)`);
      ...
      if (
        (farmerProfile.purpose === 'equipment' || farmerProfile.purpose === 'infrastructure') &&
        loan.tenure >= 48
      ) {
        score += 5;
        positiveReasons.push(`Long tenure (${loan.tenure} months) suits capital investment`);
      } else if (
        (farmerProfile.purpose === 'seeds' || farmerProfile.purpose === 'working_capital') &&
        loan.tenure <= 12
      ) {
        score += 5;
      ...
      if (loan.id.includes('subsidy') || loan.id.includes('emergency') || loan.id.includes('pm-kisan')) {
      ...
      const monthlyEMI = farmerProfile.loanAmountNeeded * 
      ...
      const totalInterest = (monthlyEMI * loan.tenure) - farmerProfile.loanAmountNeeded;
  ...
  const avgRate = recommendations.length > 0
    ? (recommendations.reduce((sum, l) => sum + l.interestRate, 0) / recommendations.length)
    : 0;
...
export function detectIntent(userMessage: string, language: 'en' | 'hi' = 'en') {
  const message = userMessage.toLowerCase().trim();
  
  // Intent patterns with keywords (supports Hindi transliteration)
  const intentPatterns = {
    scheme_query: {
      keywords: ['scheme', 'yojana', 'योजना', 'subsidy', 'government', 'sarkar', 'benefit', 'eligibility', 'qualify', 'pm-kisan', 'pmfby'],
      confidence: 0,
    },
    loan_query: {
      keywords: ['loan', 'credit', 'kcc', 'karza', 'कर्ज', 'emi', 'interest', 'borrow', 'finance', 'kisan credit'],
      confidence: 0,
    },
    insurance_query: {
      keywords: ['insurance', 'bima', 'बीमा', 'crop insurance', 'fasal bima', 'pmfby', 'protection', 'risk', 'loss'],
      confidence: 0,
    },
    price_query: {
      keywords: ['price', 'rate', 'mandi', 'मंडी', 'sell', 'market', 'bhav', 'cost', 'msp'],
      confidence: 0,
    },
    weather_query: {
      keywords: ['weather', 'rain', 'mausam', 'मौसम', 'barish', 'temperature', 'forecast', 'drought', 'flood'],
      confidence: 0,
    },
    crop_advice: {
      keywords: ['crop', 'fasal', 'फसल', 'grow', 'plant', 'seed', 'beej', 'sowing', 'harvest', 'pesticide'],
      confidence: 0,
    },
    financial_health: {
      keywords: ['health', 'score', 'status', 'swasthya', 'debt', 'income', 'savings', 'financial'],
      confidence: 0,
    },
    general_greeting: {
      keywords: ['hello', 'hi', 'namaste', 'नमस्ते', 'help', 'madad', 'मदद', 'kaise', 'how'],
      confidence: 0,
    },
  };
  ...
  for (const [intent, data] of Object.entries(intentPatterns)) {
    ...
    data.confidence = Math.min(95, (matchCount / data.keywords.length) * 100 + 
                                   (matchCount > 0 ? 20 : 0)); // Base boost if any match
  }
...
export function calculateFinancialHealth(farmerProfile: {
  ...
  let totalScore = 0;
  const factors: { category: string; score: number; weight: number; status: string }[] = [];
  ...
  if (farmerProfile.totalDebt) {
    ...
    factors.push({
      ...
      score: debtScore,
      weight: 25,
      status: dtiRatio <= 0.3 ? 'Excellent' : dtiRatio <= 0.5 ? 'Good' : 'High Debt',
    });
  } else {
    factors.push({
      category: 'Debt Management',
  ...
  const totalAssets = (farmerProfile.landValue || 0) + 
  ...
  factors.push({
    ...
    score: assetScore,
    weight: 20,
    status: totalAssets >= 500000 ? 'Strong' : totalAssets >= 100000 ? 'Moderate' : 'Weak',
  });
  ...
  if (farmerProfile.savingsAmount && farmerProfile.monthlyExpenses) {
    const monthsOfSavings = farmerProfile.savingsAmount / farmerProfile.monthlyExpenses;
    if (monthsOfSavings >= 6) savingsScore = 15;
    else if (monthsOfSavings >= 3) savingsScore = 12;
    else if (monthsOfSavings >= 1) savingsScore = 8;
    else savingsScore = 4;
  } else if (farmerProfile.savingsAmount && farmerProfile.savingsAmount > 50000) {
    savingsScore = 12;
...
export function forecastCropPrices(cropData: {
  ...
  const { cropName, currentPrice, historicalPrices = [], season, weatherCondition = 'normal' } = cropData;
  ...
  // Generate forecasts
  const forecast30 = currentPrice * trendMultiplier * seasonFactor * weatherFactor * (0.98 + Math.random() * 0.04);
  const forecast60 = forecast30 * trendMultiplier * (0.97 + Math.random() * 0.06);
  const forecast90 = forecast60 * trendMultiplier * mspFactor * (0.96 + Math.random() * 0.08);
  ...
  return {
    ...
    factors: {
      trend: trendMultiplier > 1 ? 'Upward' : trendMultiplier < 1 ? 'Downward' : 'Stable',
      seasonal: seasonFactor > 1 ? 'Peak Season' : seasonFactor < 1 ? 'Off Season' : 'Normal',
      weather: weatherCondition,
      mspSupport: mspFactor > 1 ? 'Active' : 'None',
    },
    recommendations: generatePriceRecommendations(sentiment, priceChange),
    nextUpdate: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Weekly update
  };
}

function generatePriceRecommendations(sentiment: string, priceChange: number): string[] {
