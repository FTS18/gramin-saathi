...
async function decryptSensitiveFields(data: any): Promise<any> {
  ...
  for (const [key, value] of Object.entries(data)) {
    ...
    } else if (!key.endsWith('_isEncrypted') && typeof value === 'object' && value !== null) {
      // Recursively process nested objects
...
export function secureDoc(firestore: any, path: string, ...pathSegments: string[]) {
  return doc(firestore, path, ...pathSegments);
}

// Helper function to create secure collection reference
export function secureCollection(firestore: any, path: string, ...pathSegments: string[]) {
